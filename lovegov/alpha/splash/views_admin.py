...
from lovegov.beta.modernpolitics.forms import CreateUserForm
from lovegov.beta.modernpolitics.forms import SearchUserForm
...
def adminHome(request,dict={}):
    ...
    if request.method == 'GET' and str(userProfile.username) in betaconstants.TEAM_EMAILS:
        dict['message'] = "Admins can add users here"
        getForms(dict)
        return renderToResponseCSRF('deployment/pages/admin/home.html', dict, request)
    elif request.method=='POST' and userProfile.username in betaconstants.TEAM_EMAILS:
