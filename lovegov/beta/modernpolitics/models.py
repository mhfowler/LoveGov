...
from lovegov.beta.modernpolitics import custom_fields
...
class LocationLevel(models.Model):
    ...
    level = models.CharField(max_length=1, choices=constants.LEVEL_CHOICES, default='F')
    class Meta:
        abstract = True
...
class Topic(LGModel):
    ...
    def getImageRef(self):
        return constants.MAIN_TOPICS_IMG[self.topic_text]

    def getMiniImageRef(self):
        return constants.MAIN_TOPICS_MINI_IMG[self.topic_text]

    def getUserImage(self):
        from lovegov.beta.modernpolitics.backend import getTopicImage
...
class Content(Privacy, LocationLevel):
    ...
    # FIELDS
    type = models.CharField(max_length=1, choices=constants.TYPE_CHOICES)
    ...
    #-------------------------------------------------------------------------------------------------------------------
    def getRating(self):
        votes = self.upvotes + (self.downvotes*constants.DOWNVOTE_DAMPEN)
        if votes:
...
class BasicInfo(LGModel):
    ...
    invite_message = models.CharField(max_length=10000, blank=True, default=constants.DEFAULT_INVITE_MESSAGE)
    invite_subject = models.CharField(max_length=1000, blank=True, default=constants.DEFAULT_INVITE_SUBJECT)
...
class FilterSetting(LGModel):
    ...
    algo = models.CharField(max_length=1, default='D', choices=constants.ALGO_CHOICES)
    hot_window = models.IntegerField(default=constants.HOT_WINDOW)
...
class UserProfile(FacebookProfileModel, LGModel):
    ...
    def initNotificationSettings(self):
        self.content_notification_setting = constants.DEFAULT_CONTENT_NOTIFICATIONS
        self.user_notification_setting = constants.DEFAULT_USER_NOTIFICATIONS
        self.email_notification_setting = constants.DEFAULT_EMAIL_NOTIFICATIONS
        self.save()
    ...
    def getProfileImage(self):
        from lovegov.beta.modernpolitics.backend import getDefaultImage
    ...
    #-------------------------------------------------------------------------------------------------------------------
    def getNetwork(self):
        from lovegov.beta.modernpolitics.backend import getOtherNetwork
    ...
    def joinLoveGovGroup(self):
        from lovegov.beta.modernpolitics.backend import getLoveGovGroup
    ...
    def getQuestions(self):
        ...
        sample_size = min(constants.NUM_QUESTIONS, len(unanswered_ids))
...
class UserPermission(LGModel):
    permission_type = models.CharField(max_length=1, choices=constants.PERMISSION_CHOICES)
...
class Persistent(Content):
    ...
    debate_type = models.CharField(max_length=1, choices=constants.DEBATE_CHOICES)
...
class ViewComparison(LGModel):
    ...
    def toDict(self):
        toReturn = []
        for topic_text in constants.MAIN_TOPICS:
            topic = self.bytopic.filter(topic__topic_text=topic_text)
            topicDict = {'text':topic_text,
                         'colors':constants.MAIN_TOPICS_COLORS[topic_text],
                         'mini_img':constants.MAIN_TOPICS_MINI_IMG[topic_text],
                         'order': constants.MAIN_TOPICS_CLOCKWISE_ORDER[topic_text],
                         'result':0,
                         'num_q':0}
            if topic:
                comparison = topic[0]
...
class Group(Content):
    ...
    # group type
    group_privacy = models.CharField(max_length=1,choices=constants.GROUP_PRIVACY_CHOICES, default='O')
...
class Motion(Content):
    ...
    motion_type = models.CharField(max_length=1, choices=constants.MOTION_CHOICES, default='O')
...
class Relationship(Privacy):
    ...
    when = models.DateTimeField(auto_now_add=True)
    relationship_type = models.CharField(max_length=2,choices=constants.RELATIONSHIP_CHOICES)
    #-------------------------------------------------------------------------------------------------------------------
