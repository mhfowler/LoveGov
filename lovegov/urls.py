...
urlpatterns += patterns('',
    ...
    (r'^question/(\d+)/(comment/(?P<c_id>\d+)/)$', viewWrapper(views.questionDetail, requires_login=True)),
    (r'^news/(\d+)/$', viewWrapper(views.newsDetail, requires_login=True)),
    (r'^news/(\d+)/comment/(\d+)/$', viewWrapper(views.newsDetail, requires_login=True)),
    (r'^poll/(\d+)/$', viewWrapper(views.pollDetail, requires_login=True)),
    (r'^poll/(\d+)/(comment/(?P<c_id>\d+)/)?$', viewWrapper(views.pollDetail, requires_login=True)),
    (r'^petition/(\d+)/$', viewWrapper(views.petitionDetail, requires_login=True)),
    (r'^petition/(\d+)/(comment/(?P<c_id>\d+)/)?$', viewWrapper(views.petitionDetail, requires_login=True)),
    (r'^discussion/(\d+)/$', viewWrapper(views.discussionDetail, requires_login=True)),
    (r'^discussion/(\d+)/(comment/(?P<c_id>\d+)/)?$', viewWrapper(views.discussionDetail, requires_login=True)),
