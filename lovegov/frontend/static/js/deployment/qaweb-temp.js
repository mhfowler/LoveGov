    ...
    ({
        ...
        toggleQuestion: function(node)
        {
            ...
            else
            {
                ...
                if (this.node.idImgObj) { this.node.idImgObj.attr("src",this.node.getImage('default'));}
...
var Node = Class.extend
    ({
        ...
        parseChildrenData: function()
        {
            if (this.ring==0)
            {
                ...
                for (var i=0;i<7;i++)
                {
                    ...
                    var xpos = (Math.cos(angle) * skew)+this.xpos+7.5;
                    var ypos = (Math.sin(angle) * skew)+this.ypos+7.5;
                    var topic = new Topic
                    ({
                        order:i,
                        xpos:xpos,
            ...
            else
            {
                ...
                for (var j=0; j<this.childrenData.length;j++)
                {
                    ...
                    if (j%2==0)
                    {
                        var newskew = (this.ring+1)*(90 + (this.ring*50));
                    }
                    else
                    {
                        var newskew = (this.ring+1)*(62 + (this.ring*50));
                    }
        ...
        {
            var self = this;
            this.idDivObj.bind('click',function()
            {
        ...
        {
            var self = this;
            self.idDivObj.bind('mouseenter',function()
            {
                $(this).css('cursor','pointer');
                self._mouseEnter();
            });
            self.idDivObj.bind('mouseleave',function()
            {
                self._mouseLeave();
            });
        },
...
var Question = Node.extend
    ({
        ...
        HTML_appendImg: function()
        {
            ...
            var style = 'style="position:absolute;background-color:white;width:' + this.base_width + 'px"';
...
var Topic = Node.extend
    ({
        ...
        HTML_createNodeDiv: function(self)
        {
            ...
            var topicObj = comparisonJSON['topics'][this.order];
            ...
            var img_layer = new Kinetic.Layer();
...
function loadQAWeb()
{
    ...
    setTimeout(function() {$(QAWEB_ID).scrollTop(offsety - ($(window).height()/2)+31);},150);
