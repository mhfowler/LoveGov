...
from lovegov.old.django_facebook.connect import CONNECT_ACTIONS, connect_user
from lovegov.old.django_facebook.utils import next_redirect
from lovegov.old.django_facebook.decorators import (facebook_required,
    ...
    if facebook_login:
        ...
        if graph:
            ...
            if facebook.is_authenticated():
                ...
                mylogger.error("facebook is auteheee")
                ...
                if action is CONNECT_ACTIONS.CONNECT:
                    ...
                    return user.get_profile().post_facebook_registration(
                        request)
###################################### DENIED ##########################################################################
                elif action is CONNECT_ACTIONS.DENIED:
                    return lovegov_views.login(request, message=user)
###################################### DENIED ##########################################################################
        else:
        ...
        return next_redirect(request, redirect_url="/web/")
